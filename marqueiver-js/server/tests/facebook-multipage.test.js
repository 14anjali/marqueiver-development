...
function makeStore() {
   ...
   *
   * The mirror now resolves creator OR brand via `resolveSocialProfile`, which
   * queries both models. Leaving BrandProfile real meant the genuine Mongoose
   * model ran against this file's fake `'u1'` user id and threw a CastError, so
   * the fake has to cover everything the code under test touches — not only the
   * parts this scenario exercises.
   */
  const BrandProfile = {
    findOne() { return makeQuery(() => [], true); },
  ...
  return { FacebookPage, CreatorProfile, BrandProfile, User, pages, creators, users };
}
...
const run = (handler, req) => new Promise((resolve, reject) => {
  ...
  // A watchdog, not an assertion: everything here is in memory, so the only way
  // to reach it is a handler that genuinely never settles. 15s rather than 5s
  // because a starved CI container once tripped the shorter one on a test that
  // passes on every other run.
