...
test('Policy 5.4 — three revision rounds by default, then blocked', () => {
  // Three, not two: the included-revision count was confirmed as 3, and the
  // schema default (1), this fallback (2) and the requirement (3) had all
  // drifted apart. They now come from one constant in models/Deal.js.
  assert.equal(DEFAULT_REVISION_ROUNDS, 3);
  const deal = { terms: { revisionsAllowed: 3 }, revisionCount: 0 };
  assert.equal(canRequestRevision(deal).allowed, true);
  assert.equal(canRequestRevision({ ...deal, revisionCount: 1 }).allowed, true);
  assert.equal(canRequestRevision({ ...deal, revisionCount: 2 }).allowed, true);
  assert.equal(canRequestRevision({ ...deal, revisionCount: 3 }).allowed, false);
});

test('terms may still agree a different number of revisions', () => {
  // The constant is the default, not a cap — the parties can negotiate it.
  const two = { terms: { revisionsAllowed: 2 }, revisionCount: 2 };
  assert.equal(canRequestRevision(two).allowed, false);

  const five = { terms: { revisionsAllowed: 5 }, revisionCount: 3 };
  assert.equal(canRequestRevision(five).allowed, true);
});
