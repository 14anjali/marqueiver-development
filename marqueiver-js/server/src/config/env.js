...
/**
 * Under test, ignore the developer's `.env` entirely.
 *
 * Several tests assert on configuration — which provider variables are missing,
 * whether live mode is on. Reading the local `.env` made those assertions
 * depend on whose machine ran them: the suite passed on a laptop with no
 * credentials and failed on one with a Google secret set, for the same commit.
 * A suite whose result depends on ambient configuration cannot gate a deploy,
 * which matters here because undetected deploy drift is what has broken this
 * project's integrations repeatedly.
 *
 * Tests that need a specific value set it explicitly in `process.env`, which
 * still works — this only stops the *file* from leaking in. `DOTENV_PATH`
 * remains an escape hatch for pointing a test run at a fixture env file.
 *
 * `NODE_TEST_CONTEXT` is set by `node --test` itself, so this needs no change
 * to the npm scripts and works the same on Windows, where `NODE_ENV=test cmd`
 * is not valid shell syntax. Detecting the test runner directly also means the
 * isolation cannot be lost by someone editing the test script later.
 */
const SKIP_ENV_FILE = Boolean(
    (process.env.NODE_TEST_CONTEXT || process.env.NODE_ENV === 'test')
    && !process.env.DOTENV_PATH,
);

const result = SKIP_ENV_FILE ? { parsed: {} } : dotenv.config({ path: ENV_PATH });
