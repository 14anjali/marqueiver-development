 ...
 *
 * A token that authorises nothing on *either* host, after an OAuth that
 * succeeded, is not a malformed request: node, version and API generation were
 * each varied independently and none of them moved the result. When that
 * pattern coincides with an empty grant, the honest error names the cause
 * instead of repeating Meta's `Unsupported request - method type: get`, which
 * describes the symptom and points nowhere.
 */
...
export function missingGrantError() {
  return new ApiError(403, 'INSTAGRAM_PERMISSIONS_NOT_GRANTED',
    ...
    + 'profile. This is an app-level approval, not something wrong with your account.',
    {
      ...
      howTo: [
        'Confirm the Instagram account is a Professional (Business or Creator) account.',
        'In the Meta App Dashboard, check App Review → Permissions and Features for '
        + 'instagram_business_basic — Standard Access only covers users with an app role.',
      ],
    });
}
