...
/**
 * Which Cashfree variables are missing, if any.
 *
 * Named individually because "payments are not configured" sends someone
 * hunting through seven variables, and the names are not secrets — the values
 * are, and none of them appear here.
 */
export function cashfreeConfigStatus() {
  const required = {
    CASHFREE_CLIENT_ID: env.cashfree.clientId,
    CASHFREE_CLIENT_SECRET: env.cashfree.clientSecret,
    CASHFREE_WEBHOOK_SECRET: env.cashfree.webhookSecret,
  };
  const missing = Object.entries(required).filter(([, v]) => !v).map(([k]) => k);

  return {
    configured: missing.length === 0 && env.integrationMode === 'live',
    missing,
...
/**
 * Refuse to start a production server that would take mock payments.
 *
 * Every function in this module falls back to a mock branch when the
 * credentials are absent, which is correct for local work and catastrophic in
 * production: `createEscrowOrder` returns a fake order reference, the deal
 * advances to funded, communication unlocks and the creator begins work — with
 * no money anywhere. Nothing in the response distinguishes that from a real
 * payment, so the first sign of trouble would be a payout that cannot be made.
 *
 * A server that cannot take real money must not pretend it can. Called from
 * server.js before the port is bound, so this fails the deploy rather than
 * failing a brand's first escrow.
 */
export function assertPaymentsSafeToBoot() {
  if (env.nodeEnv !== 'production') return;
  if (isLive()) return;

  const { missing } = cashfreeConfigStatus();
  throw new Error(
    'Refusing to start: NODE_ENV=production but Cashfree is not configured, so '
    + 'every payment would silently take the mock branch and deals would be '
    + 'marked funded with no money received.\n'
    + `Missing: ${missing.join(', ') || '(none — check INTEGRATION_MODE=live)'}\n`
    + `INTEGRATION_MODE is currently "${env.integrationMode}".`,
  );
}
