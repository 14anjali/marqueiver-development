...
import { authenticate, requireRole, requireAdminLevel, requireApprovedAdmin } from '../../middleware/auth.js';
...
router.use(authenticate, requireRole('admin'), requireApprovedAdmin);
...
router.get('/team/pending', requireAdminLevel(), c.listPendingAdmins);
router.post('/team/pending/:id/decide', requireAdminLevel(), validate(c.decideAdminApprovalSchema), c.decideAdminApproval);
// Campaign approval — the only route to a published campaign.
router.get('/campaigns', c.listCampaignReviewQueue);
router.post('/campaigns/:id/decide', requireAdminLevel('support'), validate(c.decideCampaignSchema), c.decideCampaign);
