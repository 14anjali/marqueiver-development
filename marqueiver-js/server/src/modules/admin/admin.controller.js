...
export const overview = catchAsync(async (_req, res) => {
    const [totalUsers, creators, brands, activeDeals, openDisputes, verifQueue, gmvAgg] = await Promise.all([
        ...
        Deal.countDocuments({ state: { $in: ['escrow_funded', 'in_progress', 'submitted', 'revision'] } }),
        Deal.countDocuments({ state: 'disputed' }),
...
export const resolveDealSchema = z.object({
    to: z.enum(['completed', 'cancelled', 'in_progress']),
    note: z.string().min(3),
});
...
export const inviteAdmin = catchAsync(async (req, res) => {
    ...
    if (user) {
        ...
        user = await User.create({ phone: b.phone, role: 'admin', adminLevel: b.adminLevel, phoneVerified: true });
    }
    await recordAudit({
...
export const bootstrapAdmin = catchAsync(async (req, res) => {
    ...
    const user = await User.create({ phone, role: 'admin', adminLevel: 'super', phoneVerified: true });
    ok(res, {
        id: user.id,
