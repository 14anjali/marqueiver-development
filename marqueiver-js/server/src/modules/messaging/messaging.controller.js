 ...
 * API directly does not bypass it (scope §13: "A user must not be able to
 * bypass the restriction by manually calling the message API").
 */
async function assertParty(dealId, userId, role, { enforceState = true } = {}) {
    ...
    // Admins can read any thread (support/dispute handling) regardless of state.
    if (enforceState && role !== 'admin' && !MESSAGING_ALLOWED_STATES.has(deal.state)) {
        throw new ApiError(
            403,
            'MESSAGING_LOCKED',
            'Messaging opens once this campaign is funded and active. Until then, use offers and counter-offers on the deal.',
        );
    }
    return deal;
}
...
export const markRead = catchAsync(async (req, res) => {
    // Previously unguarded — any authenticated user could mark another deal's
    // messages as read. Party check runs, state check does not (marking read
    // is harmless once a thread has become visible).
    await assertParty(req.params.dealId, req.auth.sub, req.auth.role, { enforceState: false });
...
export const listThreads = catchAsync(async (req, res) => {
    ...
    const threads = deals.map((d) => {
        ...
        return {
            ...
            // Lets the UI show a locked thread with an explanation instead of
            // discovering the 403 only after the user types a message.
            messagingUnlocked: MESSAGING_ALLOWED_STATES.has(d.state),
        };
