...
export const facebookCallback = catchAsync(async (req, res) => {
  ...
  const processingPromise = (async () => {
    try {
      ...
      // One Facebook Page belongs to one Marqueiver user.
      await assertNotLinkedElsewhere('facebook', profile.id, claims.sub);
      ...
      await FacebookPage.findOneAndUpdate(
        { user: claims.sub },
        {
          ...
          /**
           * The same value under its real name. Meta's Deauthorize and Data
           * Deletion callbacks identify the person by this app-scoped user id
           * and nothing else, so a connection that does not record it cannot be
           * found — and a deletion we cannot perform is one we would be
           * reporting to Meta as done while leaving the data in place.
           */
          facebookUserId: String(profile.id),
          userAccessToken: token.access_token,
          name: profile.name,   // schema field is `name`; `pageName` was silently dropped
...
/**
 * Disconnect Facebook (scope §16, A73). Mirrors `disconnectInstagram`. Removes
 * only the Facebook page record; Instagram and YouTube are unaffected even
 * though Instagram also authenticates through Meta.
 */
export const disconnectFacebook = catchAsync(async (req, res) => {
  ...
  await User.findByIdAndUpdate(
    userId,
    { $pull: { connectedAccounts: 'facebook' } },
  );
