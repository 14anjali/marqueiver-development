...
export const getPolicy = catchAsync(async (req, res) => {
    ...
    // The reader is shown "policy 07 of 15". The 15 was written into the page
    // as a literal, so publishing a sixteenth policy would have made every
    // policy page quietly wrong. It comes from the catalogue now.
    ok(res, {
        ...doc,
        route: doc.route ?? `/policies/${doc.slug}`,
        policyCount: POLICY_BY_SLUG.size,
        versions,
    });
});
