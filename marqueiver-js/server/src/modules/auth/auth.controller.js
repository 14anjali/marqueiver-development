...
export const refresh = catchAsync(async (req, res) => {
    ...
    let tv;
    try {
        ({ sub, tv } = verifyRefresh(req.body.refreshToken));
    } catch {
    ...
    /**
     * Has this token been revoked by a logout?
     *
     * A signature check only proves the server issued the token, not that it is
     * still meant to work. `tv` is compared against the account's current
     * counter, which `POST /auth/logout` increments.
     *
     * `?? 0` covers refresh tokens minted before this field existed: they carry
     * no `tv`, and an account that has never logged out is still at 0, so they
     * keep working rather than signing everyone out on deploy. The first logout
     * on an account moves it to 1 and those legacy tokens stop being accepted.
     */
    if ((tv ?? 0) !== (user.tokenVersion ?? 0)) {
        throw ApiError.unauthorized('Your session has expired. Please sign in again.');
    }
    ...
    ok(res, {
        ...
        refreshToken: signRefresh(user.id, user.tokenVersion ?? 0),
        user: userView(user),
...
/**
 * Sign out — the server side of it.
 *
 * Incrementing `tokenVersion` invalidates every refresh token issued to this
 * account. The access token the caller just used stays valid until it expires
 * (at most 15 minutes); versioning access tokens too would put a database read
 * in front of every authenticated request, and the short TTL is the bound that
 * buys instead.
 *
 * `$inc` is used rather than read-modify-write so two devices signing out at
 * the same moment cannot land on the same version and leave one of them valid.
 */
export const logout = catchAsync(async (req, res) => {
    await User.updateOne({ _id: req.auth.sub }, { $inc: { tokenVersion: 1 } });
