...
const userSchema = new Schema({
    ...
    /**
     * Refresh-token generation counter — the server-side half of logout.
     *
     * Every refresh token carries the value this held when it was issued.
     * Incrementing it invalidates all of them at once, which is what
     * `POST /auth/logout` does. Declared here because Mongoose runs in strict
     * mode: an undeclared path is silently dropped on write, so a counter that
     * is not in the schema would appear to increment and never persist.
     */
