...
export function AuthProvider({ children }) {
   ...
   * Sign out.
   *
   * Tell the server first — while the access token still exists — so it can
   * revoke this account's refresh tokens, then clear locally. Clearing first
   * would drop the credential the logout call needs, leaving a 30-day refresh
   * token alive on the server with nothing able to revoke it.
   *
   * `api.logout()` swallows its own failures, so local state is cleared even
   * when the request cannot be delivered.
   */
  const logout = useCallback(() => {
    if (store.token) void api.logout();
    // The socket authenticated with the token being discarded; leaving it open
    // would keep delivering this user's notifications after they signed out.
    disconnectSocket();
    store.clear();
    setUser(null);
    setNext(null);
