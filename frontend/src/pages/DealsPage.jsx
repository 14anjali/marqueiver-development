...
// Policy 5.1 lifecycle. `declined` and `cancelled` are styled apart because
// Policy 7.2 keeps them distinct and reporting depends on telling them apart.
const STATE_STYLE = {
  invitation: 'bg-slate-100 text-slate-600', negotiation: 'bg-amber-50 text-amber-600',
  accepted: 'bg-blue-50 text-blue-600', escrow_pending: 'bg-money-50 text-money-700',
  in_progress: 'bg-indigo-50 text-indigo-600', submitted: 'bg-cyan-50 text-cyan-600',
  revision: 'bg-orange-50 text-orange-600', resolution: 'bg-purple-50 text-purple-700',
  disputed: 'bg-rose-50 text-rose-700', completed: 'bg-emerald-50 text-emerald-600',
  declined: 'bg-slate-100 text-slate-500', cancelled: 'bg-rose-50 text-rose-600',
};
const FILTERS = ['all', 'invitation', 'negotiation', 'accepted', 'escrow_pending',
  'in_progress', 'submitted', 'revision', 'resolution', 'completed'];
