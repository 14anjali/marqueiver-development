...
import { authStep, stagger, rise, withReducedMotion, usePrefersReducedMotion } from '../../lib/motion';
 ...
 *
 *  1. **The method step guessed before the server answered.** Every
 *     availability check read `config?.methods?.x?.enabled !== false`, and
 *     `config` is null until `GET /auth/config` returns — so all three buttons
 *     appeared, and on an environment without MSG91 or Google credentials one
 *     of them then turned into "Not configured". A five-step signup is exactly
 *     where a dead button costs the most.
 *  2. **`useAuthConfig()`'s error was discarded**, so a failed config call left
 *     the page silently optimistic.
 *  3. **Steps had no exit animation.** The CSS class keyed on the step name
 *     replaced the card instantly, so "Change account type" read as a jump
 *     rather than as going back. Direction is the only signal a wizard gives
 *     about which way you moved, so it has to be visible.
 *  4. **A dead 220ms after the code verified.** `submitCode` verifies, then
 *     `setTimeout(… 220)` before advancing — during which `otp.verifying` was
 *     already false, so the form re-enabled and could be submitted a second
 *     time. `VerifyStep` now takes `busy` and holds through it.
 *  5. **The role cards were `<button>`s with no motion and no pressed state**
 *     on the one screen that is purely a choice.
 */
...
export default function SignupPage() {
  ...
  // Until the server has said which methods exist, none are drawn.
  ...
  async function submitCode(value) {
    ...
    setVerification(data.verificationToken);
    // Hold the form through the pause that lets the success tick land, so the
    // OTP boxes and the Verify button do not become live again for a fifth of
    // a second between "verified" and the next step appearing.
    setAdvancing(true);
    setTimeout(() => { setAdvancing(false); go('details'); }, 220);
  }
      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        {step === 'method' && (
          <div className="auth-card">
            ...
            {/* A config call that failed left the page pretending everything
                worked. Said out loud now, with a way to retry. */}
            <AuthError error={configError} onRetry={() => window.location.reload()} />

            <div className="space-y-3 mt-4 stagger">
              {configLoading ? (
                <div aria-busy="true" aria-live="polite" aria-label="Checking which sign-up methods are available">
...
/**
 * Step 1 — the only decision on this screen.
 *
 * The two cards were static `<button>`s with a hover colour. This is the single
 * highest-consequence click in the product — it decides which half of
 * Marqueiver you get, which policies bind you, and what onboarding asks for —
 * and it deserves to feel like picking something up rather than like following
 * a link. They lift on hover, press on tap, and arrive staggered so the eye is
 * taken to one and then the other rather than to both at once.
 */
      ...
      >
        {ROLES.map((r) => (
          <motion.button
            ...
            className="auth-tile group flex flex-col text-left"
          >
...
export function VerifyStep({ otp, onBack, onSubmit, eyebrow, extraError, busy = false }) {
  const isPhone = otp.channel === 'phone';

  /*
    `busy` is the caller's work *after* the code verifies — logging in, or
    creating the account. Without it there was a visible gap: `otp.verifying`
    went false the instant the code came back, so the button re-enabled and the
    OTP boxes became editable again while the login request was still in
    flight. A second Enter in that window fired the whole submission twice.
  */
  const working = otp.verifying || busy;

  /**
   * How many digits the server is actually sending.
   *
   * `GET /auth/config` has always returned `otp.length` and the UI has always
   * ignored it, hardcoding 6 in three places: the boxes, the submit gate and
   * the sentence telling the user what to expect. Setting `OTP_LENGTH` to
   * anything else produced a form that could never be submitted — the button
   * stayed disabled at 4 digits, and at 8 there were too few boxes to type into.
   *
   * Read here rather than passed in, so both callers of this shared step
   * (signup and CompleteAccount) get it without either having to know.
   */
  const { config, error: configError } = useAuthConfig();
  const codeLength = config?.otp?.length ?? 6;

  return (
    <div className="auth-card">
      <BackButton onClick={onBack} disabled={working}>Change {isPhone ? 'number' : 'email'}</BackButton>
      ...
      <AuthHeading
        ...
        sub={
          <>
            We sent a {codeLength}-digit code {isPhone ? 'on WhatsApp' : 'by email'} to{' '}
          ...
          {working
            ? (
              <>
                ...
                {otp.verifying ? 'Verifying…' : 'Signing you in…'}
              </>
            )
            : 'Verify and continue'}
        </button>
