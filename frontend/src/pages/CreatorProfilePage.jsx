...
import {
  Mail, MapPin, Star, Play, Send, Bookmark, Share,
  Platform, ChevRight, ChevLeft,
...
export default function CreatorProfilePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setCompat(data.compatibility || null);
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
          ...
          <div className="space-y-4">
            ...
            {/* AI compatibility — only shown to brands, real score from backend */}
            {compat && (
              <div className="card p-4">
                <h3 className="font-display font-bold text-ink mb-3">AI Compatibility</h3>
                <div className="flex items-center gap-3 mb-3">
                  <span className="font-display font-extrabold text-3xl text-brand-600">{compat.overall}%</span>
                  <span className="pill bg-avail-bg text-avail-fg">{compat.overall >= 80 ? 'Excellent Match' : compat.overall >= 60 ? 'Good Match' : 'Fair Match'}</span>
                </div>
                <div className="space-y-2.5">
                  {Object.entries(compat).filter(([k]) => k !== 'overall').map(([label, pct]) => (
                    <div key={label}>
                      <div className="flex justify-between text-xs mb-1"><span className="text-muted capitalize">{label.replace(/([A-Z])/g, ' $1')}</span><span className="font-semibold text-ink">{pct}%</span></div>
                      <div className="h-1.5 rounded-full bg-bg overflow-hidden"><div className="h-full bg-gradient-to-r from-brand-500 to-pink-500 rounded-full" style={{ width: `${pct}%` }} /></div>
                    </div>
                  ))}
                </div>
              </div>
