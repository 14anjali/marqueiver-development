...
import { AnimatedNumber, Money, Skeleton } from '../../components/feedback';
...
import { LIFECYCLE, CREATOR_STEPS, BRAND_STEPS, FEATURES, numberWord } from './content';
...
/**
 * The four numbers, counted rather than claimed.
 *
 * These used to be a literal array — `2,843` creators, `320+` campaigns,
 * `₹2Cr+` paid out — sitting directly under a headline about verified data and
 * money you can trust. Invented figures in that position are not a placeholder
 * to fix later; they are the page contradicting itself.
 *
 * They now come from `GET /api/platform/stats`, which counts non-suspended
 * creators, campaigns that actually reached creators, and escrow that has
 * actually been released. Because that is a network call on the first screen a
 * visitor sees, all three states are designed, not assumed:
 *
 *   loading  a skeleton the exact size of the figure, so the band does not
 *            change height when the numbers land and push the page down
 *   error    the whole band is removed. A landing page must never show
 *            "couldn't load" — the rest of the page still makes the argument,
 *            and a broken strip under the headline costs more than four
 *            numbers are worth
 *   empty    a brand-new install genuinely has zeros. Zeros are shown rather
 *            than hidden; the alternative is inventing numbers again
 */
function Proof() {
  ...
  // Nothing to show and nothing coming — drop the band entirely.
  ...
  const items = loading ? [] : [
    {
      key: 'creators',
      node: <AnimatedNumber value={stats.creators ?? 0} />,
      label: 'Creators on the platform',
    },
    {
      key: 'campaigns',
      node: <AnimatedNumber value={stats.campaigns ?? 0} />,
      label: 'Campaigns run',
    },
    {
      key: 'paidOut',
      node: <Money amount={stats.paidOut ?? 0} animate />,
      label: 'Paid out through escrow',
    },
    {
      key: 'commission',
      // The live rate from commission.service, so a Policy 14.7 rate change
      // reaches this page without anyone editing it.
      node: <span className="tnum">{stats.commissionPct ?? 12.5}%</span>,
      label: 'Flat commission, creator-side',
    },
  ];
  ...
  return (
    <section className="border-y border-line bg-white">
      {loading ? (
        <div
          className="container-wide py-10 grid grid-cols-2 lg:grid-cols-4 gap-8"
          aria-busy="true"
          aria-live="polite"
          aria-label="Loading platform figures"
        >
          {[0, 1, 2, 3].map((i) => (
            <div key={i}>
              {/* Sized to the real figure so nothing reflows on arrival. */}
              <Skeleton className="h-8 sm:h-9 w-28 rounded-lg" />
              <Skeleton className="h-3 w-36 mt-2.5 rounded" />
            </div>
          ))}
        </div>
      ) : (
        <Reveal className="container-wide py-10 grid grid-cols-2 lg:grid-cols-4 gap-8" each={0.06}>
          {items.map((s) => (
            <RevealItem key={s.key} variants={fadeUp}>
              <p className="stat-num text-2xl sm:text-3xl">{s.node}</p>
              <p className="text-[13px] text-muted mt-1.5 leading-snug">{s.label}</p>
            </RevealItem>
          ))}
        </Reveal>
      )}
    ...
    <section className="liquid-stage py-24 md:py-32">
      <div className="container-wide relative z-10">
        <Reveal className="max-w-2xl">
          ...
          <RevealItem variants={fadeUp}>
            {/*
              Counted, not written. The heading said "nine stages" while
              `LIFECYCLE` held ten and `LifecycleTrack` said ten — the home page
              and the how-it-works page disagreed about the product's own
              lifecycle. Deriving it means the copy cannot drift from the list
              rendered directly beneath it.
            */}
            <h2 className="display-section mt-3">
              Every collaboration follows the same {numberWord(LIFECYCLE.length)} stages
            </h2>
          </RevealItem>
          <RevealItem variants={fadeUp}>
