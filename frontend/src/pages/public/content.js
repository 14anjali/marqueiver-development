...
/**
 * Small numbers spelled out, for prose that has to agree with a list length.
 *
 * The home page said "nine stages" and the how-it-works page said "ten" while
 * `LIFECYCLE` below held ten — three places holding the same fact, two of them
 * wrong. Every such sentence now reads its number from the array.
 */
const NUMBER_WORDS = [
 ...
 * a marketing fiction. Three of these names used to be invented — `invited`,
 * `negotiating` and `escrow_funded` are not states the backend has, so the file
 * asserted a fidelity it did not have and anything keying on a state name
 * silently never matched. Every value below now appears in the `status` enum on
 * `models/Deal.js`. `chat` records whether free-text messaging is open at
...
export const LIFECYCLE = [
  ...
  },
  {
    ...
    actor: 'Brand',
    // Messaging is NOT open here. `messaging.policy.js` allows chat only from
    // `in_progress` onward — while a deal is awaiting payment the parties are
    // still limited to structured offers. This said `true`, which advertised a
    // rule the product does not implement.
    chat: false,
