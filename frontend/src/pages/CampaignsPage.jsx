                ...
                <div key={c._id} className="card overflow-hidden hover:shadow-cardhover transition">
                  <div className="p-4">
                    ...
                    {isBrand ? (
                      ...
                      <button onClick={() => apply(c._id)} disabled={appliedIds.has(c._id) || c.status !== 'open'} className="btn-brand w-full mt-3 disabled:opacity-50">
                        {appliedIds.has(c._id) ? 'Applied ✓' : 'Apply Now'}
                      </button>
                    )}
                  </div>
                </div>
