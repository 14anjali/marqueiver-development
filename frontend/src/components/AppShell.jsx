 ...
 * Role-aware nav.
 *
 * Scope §10 — a creator must not be presented with a directory of other
 * creators, so "Discover" is brand-only; a creator's discovery surface is
 * Campaigns. The backend enforces this too (discovery.routes.js) — this is
 * only the matching UI, not the control.
 */
const COMMON_NAV = [
  ['Dashboard', '/dashboard'],
  ['Campaigns', '/campaigns'],
...
export default function AppShell({ children }) {
  ...
  const navItems = isCreator
    ? [...COMMON_NAV, ...CREATOR_NAV]
    : [COMMON_NAV[0], ...BRAND_NAV, ...COMMON_NAV.slice(1)];
  ...
  return (
    <div className="min-h-screen bg-bg">
      {/* Policy 1.14 / 24 is enforced one level up, by the `Ready` route guard
          in App.jsx, which routes to /onboarding/policies whenever the server
          reports outstanding versions. It used to be a modal mounted here as
          well; having both meant two independent implementations of one rule,
          able to disagree with each other and with the API. The route guard is
          the single mechanism now, and it covers the age and verification
          requirements too rather than policy acceptance alone. */}
