import { LIFECYCLE, numberWord } from '../../pages/public/content';
 ...
 */
/**
 * The two stages where money actually moves, marked in ochre.
 *
 * This used to read `s.state === 'escrow_funded' || s.n === 9`. `escrow_funded`
 * is not a state the backend has, so that half never matched and the fallback
 * on `n === 9` coloured whichever row happened to be ninth — Payout — while
 * leaving Fund escrow, the moment the money is actually committed, unmarked.
 * Keyed on real state names now, so it survives a reordering of the list.
 */
const MONEY_STAGES = new Set(['escrow_pending', 'completed']);

export default function LifecycleTrack({ heading = true }) {
  return (
    <section className="liquid-stage liquid-stage--ink text-white">
      <div className="container-wide relative z-10 py-20 md:py-28">
        {heading && (
          <div className="max-w-2xl mb-12">
            ...
            <p className="text-white/70 mt-5 leading-relaxed max-w-prose">
              {/* Counted from the list rendered below, not written into the sentence. */}
              {numberWord(LIFECYCLE.length).replace(/^\w/, (c) => c.toUpperCase())} stages, each one a recorded state on the campaign. Both sides always know where a
            ...
            <li key={s.n} className="relative pl-8 md:pl-12 pb-10 last:pb-0">
              <span
                className={`absolute -left-[13px] top-0.5 w-6 h-6 rounded-full text-[11px] font-bold flex items-center justify-center ring-4 ring-ink tnum ${
                  MONEY_STAGES.has(s.state) ? 'bg-money-500 text-ink' : 'bg-brand-600 text-white'
                }`}
                aria-hidden="true"
              >
                {s.n}
        ...
        <p className="text-white/50 text-sm mt-12 max-w-prose leading-relaxed">
          Free-text messaging opens once escrow is funded and the campaign goes active — not while a
          payment is still pending. Everything before that stays in structured offers, so the terms
          both sides agreed to are always on the record.
        </p>
      </div>
    </section>
  );
