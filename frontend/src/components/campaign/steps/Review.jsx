...
import { Money, StatusPill, Skeleton } from '../../feedback';
import { Check, X, MapPin, Lock } from '../../icons';
import { StepIssues } from '../wizard';
import { PLATFORM_LABEL, PAYMENT_MODEL_LABEL } from '../vocab';

/**
 * Section H — the campaign exactly as a creator will read it, and what still
 * stands between it and being live.
 *
 * ── A preview, not a summary ───────────────────────────────────────────────
 *
 * The blocks below are laid out the way the campaign itself reads, rather than
 * as a list of the fields that were filled in: the point of a final step is to
 * catch the brief that is complete and still wrong — an exclusivity clause
 * nobody meant to leave on, a deadline a week earlier than intended.
 *
...
export default function Review({ value, readiness, onJump }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── the campaign as a creator reads it ── */}
      <SectionCard title="Preview" description="How this campaign appears to a creator.">
        <article className="rounded-xl3 border border-line overflow-hidden">
          <header className="bg-gradient-to-br from-brand-600 via-brand-500 to-pink-500 p-5 sm:p-6 text-white">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <h3 className="font-display font-extrabold text-xl sm:text-2xl leading-tight break-words">
              ...
              <StatusPill status={c.status || 'draft'} className="shrink-0 !bg-white/20 !text-white !border-white/30" />
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 mt-4 text-sm">
              ...
              {Number(c.budget) > 0 && (
                <span className="font-display font-bold">
                  ₹{Number(c.budget).toLocaleString('en-IN')} <span className="font-normal text-white/80">per creator</span>
                </span>
              )}
              {com.creatorCount > 0 && (
                <span className="text-white/85">{com.creatorCount} creator{com.creatorCount === 1 ? '' : 's'}</span>
              )}
            </div>
          </header>
          ...
          <div className="p-5 sm:p-6 space-y-6 bg-white">
            ...
            <Block label="Who can apply" empty={!(r.categories?.length || r.locations?.length || r.followerMin || r.languages?.length)}>
              <dl className="grid sm:grid-cols-2 gap-4">
                ...
                {(r.ageMin || r.ageMax) && (
                  <Pair label="Age" value={`${r.ageMin ?? 'any'} – ${r.ageMax ?? 'any'}`} />
                )}
              ...
              <div className="space-y-3">
                ...
                <dl className="grid sm:grid-cols-2 gap-4">
                  ...
                  {com.travel?.offered && (
                    <Pair
                      label="Travel"
                      value={com.travel.cap ? `Reimbursed up to ₹${Number(com.travel.cap).toLocaleString('en-IN')}` : 'Reimbursed'}
                    />
                  )}
  ...
  return (
    <section>
      ...
      <div className="mt-2.5">
        {empty
          ? <p className="text-sm text-muted italic">Nothing added yet.</p>
          : children}
      </div>
    </section>
  );
