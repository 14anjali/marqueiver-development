...
import { Logo } from '../ui';
import { AnimatedNumber, Money } from '../feedback';
...
/**
 * The three figures beside the sign-in form, counted rather than invented.
 *
 * These were a literal array — `2,843` creators, `320+` campaigns, `₹2Cr+`
 * paid out — the same three fictions that were on the landing page, written a
 * second time. Making up numbers next to a sign-up form is worse than making
 * them up on a marketing page: this is the moment someone decides whether the
 * marketplace is big enough to be worth joining.
 *
 * Three states, and the third is the interesting one:
 *
 *   loading  faint bars at the size of the figures, so the rail does not reflow
 *   error    the block is dropped. The aside is atmosphere — it is `aria-hidden`
 *            and carries nothing the user needs — so a failed stats call must
 *            never put an error message beside a login form
 *   zero     a brand-new deployment shows zeros rather than a flattering
 *            invention, which is the whole reason this changed
 */
function AsideStats() {
  ...
  if (!stats) {
    return (
      <div className="flex gap-10 mt-10">
        {[0, 1, 2].map((i) => (
          <div key={i}>
            {/* The shell sits on the dark stage, so the system's light-surface
                skeleton would be invisible here — a white wash at low opacity
                is the same idea in the right key. */}
            <div className="h-9 w-24 rounded-lg bg-white/10 animate-pulse" />
            <div className="h-3 w-16 rounded bg-white/10 animate-pulse mt-2" />
          </div>
        ))}
      </div>
    );
  ...
  const items = [
    { label: 'Creators', node: <AnimatedNumber value={stats.creators ?? 0} /> },
    { label: 'Campaigns', node: <AnimatedNumber value={stats.campaigns ?? 0} /> },
    { label: 'Paid out', node: <Money amount={stats.paidOut ?? 0} animate className="!text-white" /> },
  ];
    ...
    <dl className="flex gap-10 mt-10">
      {items.map((s) => (
        <div key={s.label}>
          ...
          <dd className="font-display font-extrabold text-3xl tnum text-white">{s.node}</dd>
...
export function BackButton({ onClick, children = 'Back', disabled = false }) {
  return (
    // Disabled while the step is mid-flight: going "back" during a login
    // request leaves the request running against a screen that has moved on.
    <button
      type="button"
      onClick={onClick}
      disabled={disabled}
      className="auth-btn-ghost mb-5 disabled:opacity-40 disabled:cursor-not-allowed"
    >
