  ...
  useEffect(() => {
    ...
    api.authConfig()
      .then(({ data }) => {
        if (!alive) return;
        /*
          A 2xx that carries no payload is a failure, not a slow success.

          `setConfig(undefined)` leaves `config` falsy forever, and the screens
          that read it treat "no config yet" as "still loading" — so the sign-in
          page sits on its skeleton indefinitely, with no error, no retry and no
          way to sign in. A misconfigured proxy answering /api/* with the SPA's
          own index.html produces exactly this: HTTP 200, a body that is not the
          expected JSON, and `data` undefined.

          Treating it as an error is what turns a silent hang into a screen that
          says what went wrong and offers a retry.
        */
        if (!data || typeof data !== 'object') {
          setError(new Error('The server did not return a usable sign-in configuration.'));
          return;
        }
        setConfig(data);
      })
